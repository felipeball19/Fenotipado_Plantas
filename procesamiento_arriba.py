...
def calculate_individual_plants_by_roi(mask_leaves: np.ndarray, roi_coords: Tuple) -> List[Dict[str, Any]]:
    """
    ...
    for roi_idx, (center_x, center_y, radius) in enumerate([(center_x1, center_y1, radius1), 
        ...
        if total_pixels < MIN_LEAF_AREA:  # Si no hay suficientes píxeles, saltar este ROI
            continue
        ...
        for plant_idx in range(plants_to_detect):
            ...
            # Crear ID único para cada planta
            plant_id = roi_idx * 10 + (plant_idx + 1)  # ROI 1: plantas 11, ROI 2: plantas 21, 22
            
            metrics = {
                'area_plantcv': area_plantcv,
                'perimeter': float(perimeter_opencv),
                'solidity': float(solidity_opencv),
                'roi_id': roi_idx,  # Identificador del ROI (1 o 2)
                'plant_id': plant_id,  # ID único de la planta
                'plant_in_roi': plant_idx + 1,  # Número de planta dentro del ROI (1 o 2)
                'center_x': center_x,
                'center_y': center_y,
                'radius': radius
            }
            ...
            print(f"     ROI {roi_idx}, Planta {plant_idx + 1} detectada: {area_plantcv:.0f} píxeles, solidez: {solidity_opencv:.3f}")
...
def process_image_arriba(image_path: str) -> Dict[str, Any]:
    ...
    morphological_metrics = calculate_individual_plants_by_roi(mask_leaves, roi_coords)
...
if __name__ == "__main__":
    ...
    print("   - Número de plantas, ID de planta, Timestamp, Coordenadas ROI")
    print("=" * 80)
